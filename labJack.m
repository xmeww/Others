...
%> can then send figital I/O commands and strobed words. labJack uses a 12bit
...
%>
%> 2014-06-17 modified by A Mihalik
%> added/modified functions: getDIO, getDIOValue, getDIODirection, rawRead
%>
%> It is capable to receive triggers e.g. from MRI scanner:
%>
%> >> lj.getDIO(4) % read fio4 state
%>________________________________________________________________________
%>
%> 2015-06-29 modified by A Mihalik
%> modified functions: open, prepareStrobe, strobeWord
%>
%> Sending triggers works on Windows on 8 bits (EIO0-7) with the same syntax 
%> as on Mac. 
%>
% ========================================================================
            ...
            if obj.silentMode == false || isempty(obj.handle)
                if ismac || isunix
                    if ~libisloaded('liblabjackusb')
                        try
                            loadlibrary('/usr/local/lib/liblabjackusb', '/usr/local/include/labjackusb.h');
                        catch ME
                ...
                elseif ispc
                    % MATLAB_LJUD is needed under Windows. It can be downloaded from
                    % http://labjack.com/support/ud/examples/matlab (Old Driver and Examples)
                    % It includes easy to use functions, samples and a LabJackUD_DoublePtr
                    % header. MATLAB_LJUD and the files from LabJack Windows installation should be
                    % found under 'C:\Program Files (x86)\LabJack (otherwise change path)
                    % See http://www.egmont.com.pl/labjack/instrukcje/LabJackUD_Driver_For_Windows.pdf
                    % for more information how to use LabJack under Windows. NOTE: the current 
                    % impementation has only a minimal functionality under Windows, however, it 
                    % can be supplemented easily with MATLAB_LJUD functions.
%                     addpath(genpath('C:\Program Files (x86)\LabJack'));
                    addpath(fullfile('C:\Program Files (x86)\LabJack', 'Drivers', '64bit'));
                    if (libisloaded('labjackud')) || (libisloaded('labjackud_doublePtr'))
                        % Libraries already loaded
                    else
                        try
                            loadlibrary('labjackud');
                        ...
                        if verLessThan('matlab', '7.0') % not tested
                            obj.functionList = libmethods('labjackud');
                            % libmethods('labjackud_doublePtr');
                        else % version 7.0 and newer
                            obj.functionList = libfunctions('labjackud', '-full');
                            % libfunctions('labjackud_doublePtr', '-full');
                        end
                    end  
                    if obj.deviceID ~= 3 % current implementation tested only on U3 device under Windows
                        obj.salutation('open method', 'No LabJack U3 devices found, going into silent mode');
                    ...
                    [error, obj.devCount] = calllib('labjackud', 'ListAll', 3, 1, 0, 0, 0, 0); % for LJ_dtU3, LJ_ctUSB                    
                    if error
                        calllib('labjackud', 'ErrorToString', error, '')
                    end
                end
                obj.inp=zeros(10,1);
                if ismac || isunix
                    ...
                    if obj.validHandle() % double-check if handle is really valid
                        obj.version = calllib('liblabjackusb', 'LJUSB_GetDeviceDescriptorReleaseNumber', obj.handle);
                    else
                        obj.handle = [];
                    end
                elseif ispc
                    [error, address, obj.handle] = calllib('labjackud', 'OpenLabJack', 3, 1, '1', 1, 0); % for LJ_dtU3, LJ_ctUSB
                    if error
                        obj.handle = [];
                    end
                end
		...
		function close(obj)
			if ~isempty(obj.handle)
                if ismac || isunix
                    if obj.validHandle() %double-check we still have valid handle
                        calllib('liblabjackusb','LJUSB_CloseDevice',obj.handle);
                    end
                else
                    % no direct close method under Windows
                end
		...
		%> @param count
		% ===================================================================
		function [in, byteout] = rawRead(obj,bytein,count)
			if ~exist('bytein','var')
				bytein = zeros(10,1);
			end
			...
			[in, obj.handle, byteout] =  calllib('liblabjackusb', 'LJUSB_ReadTO', obj.handle, bytein, count, obj.timeOut);
        ...
        function waitShort(obj,time)
            ...
            elseif ispc
                obj.salutation('waitShort', 'Method not implemented under Windows', true)
            end
        end
		...
		function waitLong(obj,time)
            ...
            elseif ispc
                obj.salutation('waitLong', 'Method not implemented under Windows', true)
            end
        end
        ...
        function timedTTL(obj,line,time)
            ...
            elseif ispc    
                obj.salutation('timedTTL', 'Method not implemented under Windows', true)
            end
        end
		...
		function setDIO(obj,value,mask,valuedir,maskdir)
            ...
            elseif ispc
                obj.salutation('setDIO', 'Method not implemented under Windows', true)
            end
		end
        ...
        function setDIODirection(obj,value,mask)
            ...
            elseif ispc
                obj.salutation('setDIODirection', 'Method not implemented under Windows', true)
            end
        end
        ...
        function setDIOValue(obj,value,mask)
            ...
            elseif ispc
                obj.salutation('setDIOValue', 'Method not implemented under Windows', true)
            end
        end
        ...
        function prepareStrobe(obj,value,mask,sendNow)
            ...
            elseif ispc
                % only EIO0-7 is implemented, however, EIO1-3 could be added with prepareWords (see above)
                if value > 255
                    obj.salutation('prepareStrobe', 'Use of EIO1-3 not implemented under Windows', true)
                else
                    error = calllib('labjackud', 'AddRequest', obj.handle, 45, 8, value, 9, 0); % LJ_ioPUT_DIGITAL_PORT, strobe number + strobe line low
                    if error
                        calllib('labjackud', 'ErrorToString', error, '')
                    end
                    error = calllib('labjackud', 'AddRequest', obj.handle, 45, 8, value+256, 9, 0); % LJ_ioPUT_DIGITAL_PORT, strobe number + strobe line high
                    if error
                        calllib('labjackud', 'ErrorToString', error, '')
                    end
                    error = calllib('labjackud', 'AddRequest', obj.handle, 70, 0, 1000, 0, 0); % LJ_ioPUT_WAIT for 1000 microseconds
                    if error
                        calllib('labjackud', 'ErrorToString', error, '')
                    end
                    error = calllib('labjackud', 'AddRequest', obj.handle, 45, 8, 0, 9, 0); % LJ_ioPUT_DIGITAL_PORT, set all bits to 0
                    if error
                        calllib('labjackud', 'ErrorToString', error, '')
                    end
                end
		...
		function strobeWord(obj)
            ...
            elseif ispc
                error = calllib('labjackud', 'Go');
                if error
                    calllib('labjackud', 'ErrorToString', error, '')
                end
%                 error = calllib('labjackud','GetResult', obj.handle, 45, 8, 0); % LJ_ioPUT_DIGITAL_PORT
%                 if error
%                     calllib('labjackud', 'ErrorToString', error, '')
%                 end
            end
		...
		function setFIO(obj,val,line)
            ...
            elseif ispc
                obj.salutation('setFIO', 'Method not implemented under Windows', true)
            end
        end

        % ===================================================================
		%> @brief getDIO
		%> getDIO sets the direction for any FIO, EIO or CIO as read and gets
		%> its state driven by a signal (and NOT the value that was set by
        %> e.g. setFIO earlier).
        %> CAUTION: default state is 1 when GND is not connected (0 otherwise)
		%>
		%>	@param line 0-7=FIO, 8-15=EIO, or 16-19=CIO
		% ===================================================================
        function getDIO(obj,line)
            if ismac || isunix
                if ~exist('line','var');fprintf('\ngetDIO Input options: \n\t\tline\n\n');return;end
                if obj.silentMode == false && obj.vHandle == 1
                    cmd=zeros(12,1);
                    cmd(2) = 248; %command byte for feedback command (f8 in hex)
                    cmd(3) = (length(cmd)-6)/2;
                    cmd(8) = 13; %IOType for BitDirWrite (0d in hex)
                    cmd(9) = line;
                    cmd(10) = 10; %IOType for BitStateRead (0a in hex)
                    cmd(11) = line;
                    
                    obj.command = obj.checksum(cmd,'extended');
                    obj.outp = obj.rawWrite(obj.command);
                    [obj.inp, byteout] = obj.rawRead(zeros(1,10),10);
                    val = byteout(10);
                    if line < 8
                        myname = ['fio' num2str(line)];
                        obj.(['fio' num2str(line)]) = val;
                    elseif line > 15
                        myname = ['cio' num2str(line-16)];
                    else
                        myname = ['eio' num2str(line-8)];
                    end
                    if val == 1
                        obj.salutation('getDIO',[myname ' is HIGH'])
                    else
                        obj.salutation('getDIO',[myname ' is LOW'])
                    end
                end
            elseif ispc
                obj.salutation('getDIO', 'Method not implemented under Windows', true)
            end
        end
           
        % ===================================================================
		%> @brief Get DIO value
		%> getDIOValue gets the state of any FIO, EIO or CIO (HIGH or LOW), 
        %> independently whether the direction of the DIO is read or write 
        %> (see getDIODirection).
		%>
		%>	@param line 0-7=FIO, 8-15=EIO, or 16-19=CIO
		% ===================================================================
		function getDIOValue(obj,line)
            if ismac || isunix
            if ~exist('line','var');fprintf('\ngetFIOValue Input options: \n\t\tline\n\n');return;end
            if obj.silentMode == false && obj.vHandle == 1
                cmd=zeros(10,1);
                cmd(2) = 248; %command byte for feedback command (f8 in hex)
                cmd(3) = (length(cmd)-6)/2;
                cmd(8) = 10; %IOType for BitStateRead (0a in hex)
                cmd(9) = line;
                ...
                if val == 1
                    obj.salutation('getDIOValue',[myname ' is HIGH'])
                else
                    obj.salutation('getDIOValue',[myname ' is LOW'])
                end
            end
            elseif ispc
               obj.salutation('getDIOValue', 'Method not implemented under Windows', true) 
            end
        end 
        
        % ===================================================================
		%> @brief Get DIO direction
		%> getDIODirection gets the direction of any FIO, EIO or CIO (read 
        %> or write), however, it does not change it. Direction can be
        %> changed with setDIODirection or it is inherently changed using 
        %> setFIO and readFIO.
		%>
		%>	@param line 0-7=FIO, 8-15=EIO, or 16-19=CIO
		% ===================================================================
		function getDIODirection(obj,line)
            if ismac || isunix
                if ~exist('line','var');fprintf('\ngetDIODirection Input options: \n\t\tline\n\n');return;end
                if obj.silentMode == false && obj.vHandle == 1
                    cmd=zeros(10,1);
                    cmd(2) = 248; %command byte for feedback command (f8 in hex)
                    cmd(3) = (length(cmd)-6)/2;
                    cmd(8) = 12; %IOType for BitDirRead (0a in hex)
                    cmd(9) = line;
                    ...
                    if val == 1
                        obj.salutation('getDIODirection',[myname ' direction is WRITE'])
                    else
                        obj.salutation('getDIODirection',[myname ' direction is READ'])
                    end
                end
            elseif ispc
                obj.salutation('getDIODirection', 'Method not implemented under Windows', true)
            end
        end 
		...
		function ledON(obj)
            ...
            elseif ispc
                obj.salutation('ledON', 'Method not implemented under Windows', true)
            end
		end
		...
		function ledOFF(obj)
            ...
            elseif ispc
                obj.salutation('ledOFF', 'Method not implemented under Windows', true)
            end
        end
		...
		function reset(obj,resetType)
            ...
            elseif ispc
                obj.salutation('reset', 'Method not implemented under Windows', true)
            end
		end
